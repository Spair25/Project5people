...
magician=Player("magician.png",50,500,50,10)
enemy = Enemy("enemi.png", 725, 486, 5, 0)
enemy1 = Enemy("enemi.png", 743, 286, 5, 0)
x=0
y=0
for plt in level:
    x=0
    for p in plt:
        if p=="-":
            platform=GameSprite('platform.png',x,y,50,0)
            platforms.add(platform)
        if p=="0":
            portal=GameSprite('portale.png',x,y,50,0)
            portals.add(portal)
        if p=="u":
            coldron=GameSprite('coldron.png',x,y,50,0)
            coldrons.add(coldron)
        if p=="H":
            leder=GameSprite('leder.png',x,y,50,0)
            leders.add(leder)
        if p=="8":
            glass=GameSprite('glass.png',x,y,50,0)
            glasses.add(glass)
        if p=="+":
            clever=GameSprite('clever.png',x,y,50,0)
            clevers.add(clever)
        x+=50
    y+=50
...
background_m=transform.scale(image.load("background.jpg"),(800,600))
...
menu = True
game = False
main_m = mixer.Sound("MenuSound.ogg")
while menu:
    for e in event.get():
        if e.type==QUIT:
            menu=False
